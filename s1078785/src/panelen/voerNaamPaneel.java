...
public class voerNaamPaneel extends JPanel implements MouseListener,ItemListener{

	/**
	 * vervolg van 1 speler 
	 *  
	
			+----------+    +------+---+    +----------+
			|          |    |      |   |    |          |
			| Nieuwspel| => | Spel | 1 | => | Volgende |
			|          |    |      |   |    |    JP    |
			+----------+    +------+---+    +----------+
			
			<<< FXconcept 1 = javax.swing.Timer voor paneel inschuiven 
			van links in de vorige paneel implementeren
			
			acties: Speler voert naam 1 en druk op start
			Start =  this.verwijderen & spel.start();
			
			startButton doet static initialiseren:
			<< FXconcept 2 = (in de tussentijd word er een ander scherm weergegeven(opt:FXconcept1)
			
			
			static{
				//controleren of bord leeg is
				 * 
				 * kaarten plaatsen op bord
				 * controleren mogelijkheden om de eerste set te creeren
				 * Threads, timers
				 * De juiste paneel laden voor aantalspelers(dit geval 1) 
				 * object schrijven bij highscores arraylist
	...
	public voerNaamPaneel(Main main){
		this.main = main;
		this.setLayout(null);
